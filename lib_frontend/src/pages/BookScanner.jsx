...
export default function BookScanner() {
  ...
  const startScanner = () => {
    ...
    setTimeout(() => {
      try {
        ...
        console.error('Failed to create html5-qrcode scanner:', err)
  ...
  const stopScanner = () => {
    if (scannerRef.current) {
      scannerRef.current.clear().catch((err) => console.warn('Scanner clear warning:', err))
  ...
  return (
    <div className="min-h-screen w-full bg-slate-900 text-white flex flex-col">
      <header className="sticky top-0 z-20 flex h-16 items-center border-b border-slate-800 bg-slate-900/90 px-4 backdrop-blur-md">
      ...
      <main className="flex-1 flex flex-col px-4 py-6 max-w-md mx-auto w-full justify-center">
        {scanStatus === 'scanning' && (
          <div className="relative flex flex-col items-center gap-6">
            <div className="relative w-full aspect-square max-w-[320px] rounded-2xl overflow-hidden border border-slate-800 bg-black/80 flex items-center justify-center">
              ...
              <div id="scanner-view" className="w-full h-full text-slate-800" />
            </div>

            <div className="w-full max-w-[320px] bg-slate-800/80 border border-slate-700/50 p-4 rounded-2xl text-center">
        ...
        {scanStatus === 'searching' && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1.5">Querying central inventory database</p>
          </div>
        )}

        {scanStatus === 'found' && bookDetails && (
          <div className="flex flex-col gap-6 animate-in fade-in duration-200">
            <div className="rounded-2xl border border-white/10 bg-slate-800/80 p-6 shadow-xl text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">Author: {bookDetails.author}</p>
              
              <div className="mt-4 border-t border-slate-700/50 pt-4 flex flex-col gap-2.5 text-xs text-left">
                <div className="flex justify-between">
                  <span className="text-slate-400">ISBN-13 Code</span>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="w-full rounded-xl border border-slate-700 py-3 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white transition"
              >
                Cancel & Rescan
              </button>
            </div>
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center animate-in zoom-in-95 duration-200">
            ...
            <div className="mt-8 flex flex-col gap-3 w-full">
              <button
                type="button"
                onClick={() => navigate('/student')}
                className="w-full rounded-xl bg-slate-800 py-3.5 text-xs font-semibold hover:bg-slate-700 transition"
              >
                Return to Dashboard
              </button>
        ...
        {scanStatus === 'error' && (
          <div className="flex flex-col gap-6 animate-in fade-in duration-200">
            <div className="rounded-2xl border border-red-500/10 bg-red-500/5 p-6 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1.5">{errorMsg || 'An unknown error occurred.'}</p>
            </div>
        ...
        {scanStatus === 'scanning' && (
          <div className="mt-8 border-t border-slate-800 pt-6">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider text-center">Cannot Scan? Entry Fallback</h4>
            <form onSubmit={handleManualSearch} className="mt-3 flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-800 bg-slate-900/60 py-3.5 pl-10 pr-4 text-xs text-white placeholder:text-slate-500 outline-none focus:border-blue-500"
                />
              </div>
              <button
                type="submit"
                className="rounded-xl bg-slate-800 px-4 text-xs font-semibold hover:bg-slate-700 transition"
              >
                Search
              </button>
            </form>
